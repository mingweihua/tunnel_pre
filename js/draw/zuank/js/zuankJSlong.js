...
var GetEGHOLEUrl = "http://localhost:8080/zuanKongGetEGHOLE?"+stringHoleUrl;
var GetEGHOLELayerUrl = "http://localhost:8080/zuanKongGetEGHOLELayer?"+stringHoleUrl;
var GetEgstdpenetrationUrl = "http://localhost:8080/getEgstdpenetration?"+stringHoleUrl;
/*ar GetEGHOLEUrl = window.location.origin+"/taizhou/zuanKongGetEGHOLE?"+stringHoleUrl;
	...
	for(var y=1;y<=YS;y++){
		...
		for(i=j;i<o2.length;i++){
			...
			if(SEL/=null){
				...
				switch(b)
				{
					case "7":o2[i].stratlevel ="5-3";break;
					case "8":o2[i].stratlevel ="5-2";break;
					case "10":o2[i].stratlevel ="1";break;
				}
			}else{
				j=i;
				//console.log(j);
...
function huaTu(){		   
	...
	/*----- 长条钻孔柱状图(页数永远一页)-----*/
	var buchong = CDK*MeiMi+o2.length*3-(1478-115) + 20;//20是让它多一点免得底部出问题
	var heightNewBig = 1754 + buchong;
	var heightNew = 1478 + buchong;
	var heightNewBigStr = heightNewBig+'px';
	var heightNewStr = heightNew+'px';
	document.getElementById('divA1').style.height = heightNewBigStr;
	document.getElementById('divA11').style.height = heightNewStr;
	document.getElementById('divA12').style.height = heightNewStr;
	document.getElementById('divA13').style.height = heightNewStr;
	document.getElementById('canvasDiv').style.height = heightNewStr;
	document.getElementById('mycanvas1').height = heightNew;
	document.getElementById('mycanvas1').style.height = heightNewStr;
	/*----- 长条钻孔柱状图-----*/
	
	//------计算用多少页（总的只有6页）--------
	var YeShu=(CDK*MeiMi+o2.length*3)/(heightNew-115); 
	...
	$.ajax({
			...
			success: function (stdpdata) {
					...
					if(stdpdata.length>0){
						  ...
						  var BGyeshu=(BGCDK*MeiMi+13)/(heightNew-115);       //表格底线在字体的下部，字体为20px，所以预留13px，保证字体中线能对准.
					...
					for(var y=1;y<=YS;y++){
						...
						ctxl.beginPath();
						ctxl.moveTo(0,H1+115.2+0.3);
						ctxl.lineTo(6.19,H2+115.2+0.3);
						...
						ctxl.beginPath();
						ctxl.moveTo(0,115.2+0.3);
						ctxl.lineTo(6.19,115.2+0.3);
						ctxl.strokeStyle="black";
						ctxl.lineWidth=1;
						ctxl.closePath();
						ctxl.stroke();
					    ...
					    if(i==o2.length-1){
						   if(H1<(heightNew-115)){
							 var i2=i+1
							 var zengjia=heightNew-115-H1-3;
							 var zengjia2=heightNew-115-H2-3;
							 //console.log(zengjia);
						...
						//------------------当层数较多时，转到下一页--------------------------
					      if(H1>=(heightNew-115)){
							 //console.log(H1);
							jianshao=H1-(heightNew-115);
							jianshao2=H2-(heightNew-115);
							//console.log(H1);
							H1=H1-h1;
							H2=H2-h2;
							//console.log(H1);
					...
					if(BGYS==0){
						for(var y=1;y<=YS;y++){
							...
							if(y==1){
								document.getElementById("ZDSD"+y).height=heightNew-115-11-3+"px";
								document.getElementById("ZDSD"+y).height=heightNew-115-11-3+"px";
							}else{
								document.getElementById("ZDSD"+y).height=heightNew-115-3+"px";
								document.getElementById("ZDSD"+y).height=heightNew-115-3+"px";
							}
						...
						for(var y=1;y<=YS;y++){
							if(y>BGYS){
								...
								document.getElementById("ZDSD"+y+0).height=heightNew-115-3+"px";
								document.getElementById("ZDSD"+y+0).height=heightNew-115-3+"px";
							}else{				
								...
								for(k=l;k<stdpdata.length;k++){
									...
									if(k==stdpdata.length-1){
									   if(H1<(heightNew-115-12)){
										 var k2=k+1
										 var zengjia=heightNew-115-H1;
										 //console.log(zengjia);
										 if(y==1){
