	...
	function buildGrid(target){
		...
		$(target).datagrid($.extend({}, opts, {
			onDblClickCell:function(index,field){console.log("双击事件");
				...
				//console.log(opts);
				if(opts.editIndex!=index&&opts.editIndex>=0){
			...
			onAfterEdit: function(index, row){//console.log('onAfterEdit事件');
				...
				if(!row.isNewRecord){//console.log("测试撤销");
					if(!compareTwoObject($("#dg").datagrid('options').origins.rows[index],row)){//opts.oringinOfTargetedRow
						$(target).edatagrid('editRow',index);
						$.messager.progress('close');
						$.messager.alert(null,'您并没有对该数据进行改动',"info",function(){
			...
			/*	
				...
				if(!row.isNewRecord){
					$.messager.prompt(null,"请输入操作理由.",function (r){
						if (r){//&&r.trim().length>=0
							$.messager.confirm(null,'修改理由不可更改，是否提交？',function(res){
								//$.messager.progress('close');
								if(res){
									if(r){row.THE_OPERATION_REASON=r.trim();}
								...
								/*	$(target).datagrid('updateRow', {
										index: index,
										row: $("#dg").datagrid('options').origins[index]
									});*/
									$(target).edatagrid('editRow',index);
									$.messager.progress('close');
								}
...
//				}
		...
		saveRow: function(jq){console.log('saveRow');
			...
			return jq.each(function(){
				...
				if (opts.editIndex >= 0){
					var row=dg.datagrid('getRows')[opts.editIndex];//console.log(row);
